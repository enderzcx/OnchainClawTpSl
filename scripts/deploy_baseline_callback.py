...
DEFAULT_CONTRACT_PATH = "aa-v2/contracts/reactive/baseline/BaselineStopTakeProfitCallback.sol"
DEFAULT_CONTRACT_NAME = "BaselineStopTakeProfitCallback"
DEFAULT_CALLBACK_SENDER = "0xdb81A196A0dF9Ef974C9430495a09B6d535fAc48"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Deploy the official baseline callback on BSC.")
    ...
    parser.add_argument("--callback-sender", default=DEFAULT_CALLBACK_SENDER)
    ...
    deploy_tx = contract.constructor(
        ...
        Web3.to_checksum_address(args.callback_sender),
        Web3.to_checksum_address(args.router),
    ).build_transaction(
        {
            "chainId": 56,
    ...
    if receipt.status != 1:
        raise RuntimeError("baseline callback deployment failed on-chain")
    print(json.dumps({
        "contract_address": receipt.contractAddress,
        "tx_hash": tx_hash.hex(),
        "gas_used": int(receipt.gasUsed),
        "owner_address": args.owner_address,
        "callback_sender": args.callback_sender,
        "router": args.router,
